# 💊 営業分析ダッシュボード 仕様書 (Ver. 2.5)

本ドキュメントは、営業管理ダッシュボード（フロントエンド・モックアップ）の仕様、データ構造、数式モデル、およびUIコンポーネントの挙動を定義した参照用ドキュメントです。

> [!WARNING]
> **更新ルール**
> 本ダッシュボードの計算式、データ定義、UI構造を変更した場合は、`README.md` を更新して実装と整合させてください。
...
6. [仕様書アップデート手順](#6-仕様書アップデート手順)

---

## 1. システム概要 ＆ クイックスタート

本ダッシュボードは、営業活動における主要KPIの追跡から、先行指標を用いた将来予測、散布図によるメンバー相関分析、AI風指導アクション、市場需給分析までを網羅した高機能シングルページアプリケーション（SPA）です。

Ver. 2.5 では、主要画面を `ファネル分析`、`売上進捗`、`市況分析` の3ページに整理しました。ファネル分析ページの内部に「全体」「チーム」「メンバー」の3レイヤー切り替えを持たせています。

1. `ファネル分析`: 全体、チーム、メンバーの3レイヤーで、同じ7段階ファネル分析と配下一覧を表示する。
2. `売上進捗`: 年度累計の売上KPI、受注日ベースの日次売上積み上げ、売上詳細比較マトリクス（チーム別・個人別・エリア別、かつソート対応）、平均単価、失注・辞退理由を表示する。
3. `市況分析`: エリア×カテゴリの求人倍率ヒートマップ、注力市況サマリー、条件別成約シェアを表示する。

**Ver. 2.5 での変更: 分析ページの再分離**
- **ファネル分析の集約**: 旧「全体」「チーム」「メンバー」の3ページを、`ファネル分析` ページ内のレイヤー切り替えとして統合します。
- **売上進捗ページの追加**: `dashboardData.kpis`、`salesOrders`、`teamsData`、`unitPriceTrend`、`lossReasons` を使って、年度累計の売上達成状況と当月の日次積み上げをファネルとは別軸で確認できます。
- **市況分析ページの追加**: `marketHeatmap` と `contractTypes` を使って、求人倍率の偏りと成約構成を確認できます。
- **不要要素の整理**: 個別処方箋、設定画面、旧シミュレーターなどは引き続き主要導線から外し、3つの分析ページに集中します。
- **同一UIでの粒度切り替え**: 全体、チーム、メンバーの各レイヤーで、既存のチーム分析画面にあったファネル可視化とボトルネック診断を共通利用します。
- **配下一覧の固定配置**: 全体ではチーム一覧、チームではメンバー一覧、メンバーでは全メンバー一覧を表示し、クリックで該当レイヤーの分析へ遷移します。
- **個人ファネルの換算**: メンバー分析では、架電数、接続率、面談設定率、面談数、提案数、推薦数、面接設定数、決定数から分析用ファネルを生成します。
- **プロセス別行動チェック**: ファネルの各プロセスカードをクリックすると、そのプロセスを成立させるために必要な行動の実施状況が展開表示されます。展開パネルの下に次工程への全工程比が続くため、ボトルネックが悪いときに原因行動ができているかを確認できます。
- **行動チェック一括開閉**: ファネル分析ページ上部の「行動を全て開く」「閉じる」ボタンで、表示中レイヤーの全プロセス行動チェックを一括で開閉できます。
...
- **売上進捗グラフの日次積み上げ化**: `salesOrders` の `orderDate` をもとに、当月の受注済み売上と求職者書類回収済み売上を系列分けした積み上げ棒グラフで表示します。
- **チーム別カードの連動**: 期間選択に応じて中段の「チーム別横並び比較」の実績・目標値も multiplier に基づき同期再計算されます。

**Ver. 4.0 での拡張機能: 市場調査（都道府県別・エリア別）の強化 ＆ エリア定義管理画面の新規搭載**
- **市場調査の多角化（サブタブ化）**: 「市場・戦略」画面内に、地域別の詳細ヒートマップを表示する「地域別」サブタブと、定義に基づいたエリアの需給や比較を表示する「エリア別」サブタブを搭載。
- **エリア別データ動的集計エンジン**: ユーザーが編集したエリア定義に基づき、各エリアの「総件数」「総対象数」「平均倍率」をリアルタイムに合算・加重平均して再計算・グラフ描画。
- **カテゴリヒートマップの動的連動**: エリア定義が変更されると、「エリア別カテゴリ×倍率ヒートマップ」も所属データを元にリアルタイムに再集計・再描画されます。
- **永続化とリセット機能**: 変更したエリア設定は `localStorage` に即時記憶・永続化され、リロード後も有効です。「デフォルト設定に戻す」ボタンでいつでも標準の地域区分へリセット可能です。
...
├── data.js      # データベースに相当するモックデータ定義
├── app.js       # ApexCharts描画・シミュレーター方程式・タブ＆ポップオーバー制御
└── README.md    # 仕様の参照用ドキュメント
```
...
司令塔の「チーム別ファネル比較」で利用される、各管理グループごとのファネル進捗データおよび週次行動データ（目標 vs 実績）です。
...
* **横並び比較のソート機能**:
  * 各プロセス名（新規登録、新規面談設定など）の左カラムヘッダーをクリックすると、その工程の「実績値」の大きさでアイテムがソートされます（前工程比ではなく指標そのものの大小でソート）。
  * 各「前工程比」の行の最も左に配置されたソートボタンをクリックすると、その工程の「前工程比（移行率）」のパーセンテージの大小でアイテムがソートされます。
  * 現在のソート状態（昇順・降順）は、ヘッダーに配置された矢印アイコンの向きで直感的に判別できます。
...
4. **チーム別パフォーマンス横並び比較セクション**:
   * 司令塔下部に、全8チームのファネル進捗および週次行動量の目標 vs 実績が表示されます。
   ...
   * **詳細分析への連携**: カード下部の「このチームのファネルを見る」ボタンをクリックすると、左サイドバーの「管理チーム（`teamSelector`）」が自動で該当チームに切り替わり、`チーム分析` タブへ遷移して、最上部へスムーズに自動スクロール（スマートスクロール）されます。
...
仕様変更（数式の修正、変数の追加、新規タブの実装など）を行う際は、仕様書とコードの乖離を防ぐために次の点を確認してください。
...
### リマインダーの配置場所
更新を忘れないよう、システム内の以下の箇所に注意書きが埋め込まれています。
