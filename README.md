...
Ver. 2.5 では、主要画面を `ファネル分析` と `クロス集計` の2ページに整理しました。ファネル分析ページの内部に「全体」「チーム」「メンバー」の3レイヤー切り替えを持たせています。

1. `ファネル分析`: 全体、チーム、メンバーの3レイヤーで、同じ7段階ファネル分析と配下一覧を表示する。
2. `クロス集計`: 全登録データからリアルタイムクロス集計を行い、プロセス間の関連性を多角的に分析する。

**Ver. 2.5 での変更: 分析ページの整理**
- **ファネル分析の集約**: 旧「全体」「チーム」「メンバー」の3ページを、`ファネル分析` ページ内のレイヤー切り替えとして統合します。
- **不要要素の整理**: 個別処方箋、設定画面、旧シミュレーターなどは引き続き主要導線から外し、分析ページに集中します。
...
- **プロセス別行動チェック**: ファネル of 各プロセスカードをクリックすると、そのプロセスを成立させるために必要な行動の実施状況が展開表示されます。展開パネルの下に次工程への全工程比が続くため、ボトルネックが悪いときに原因行動ができているかを確認できます。
